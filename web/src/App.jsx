...
import { applyDealOpts, defaultDealOpts } from "./lib/dealSort";
...
// "할인 중" 섹션: 정렬·필터 바 + 거른 목록.
function DealsView({ state, opts, onOptsChange, onCardClick }) {
  const filtered = state.status === "ok" && opts ? applyDealOpts(state.rows, opts) : [];
  return (
    <>
      <h2>
        지금 할인 중인 게임 <span className="cnt">{state.status === "ok" ? filtered.length : "·"}</span>
      </h2>
      {opts && <DealControls opts={opts} onChange={onOptsChange} />}
      {state.status === "loading" && <div className="loading">불러오는 중…</div>}
...
export default function App() {
  ...
  return (
    <WishlistProvider value={wl}>
      <Header lowCount={lowCount} />

      <div className="wrap">
        <Hero query={query} onQueryChange={setQuery} />
        <Tabs active={searching ? null : tab} onChange={onTabChange} wishCount={wl.ids.length} />
